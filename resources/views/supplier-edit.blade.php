...
<div class="container" >
    ...
    <!-- ページタイトル -->
       <!-- ページタイトル -->
    <form action="{{ route('supplier-edit.edit', $supplier->id) }}" method="post">
        {{ csrf_field() }}
        ...
        @if($errors->has('street_address_1') || $errors->has('street_address_2') || $errors->has('street_address_3') || $errors->has('postal_code'))
            <?php
                $street_address_errors = $errors->get('postal_code');
                $street_address_errors += $errors->get('street_address_1');
                $street_address_errors += $errors->get('street_address_2');
                $street_address_errors += $errors->get('street_address_3');
            ?>
            @foreach ($street_address_errors as $street_address_error)
            <span class="invalid-feedback" role="alert">
                <strong>{{ $street_address_error }}</strong>
            </span>
            @endforeach
         @endif
    ...
    $('#inputGroup').change(function() {
        if($('option:selected').val() == 'etc'){
            $('#inputGroup').html('<input type="text" class="form-control col-3 @error('phone_number_1') is-invalid @enderror" placeholder="" aria-label="" name="phone_number_1" aria-describedby="addon-wrapping">' +
                '<input type="text" class="form-control col-9 @error('phone_number_2') is-invalid @enderror" placeholder="" aria-label=""  name="phone_number_2" aria-describedby="addon-wrapping">'+
                '<input type="text" class="form-control col-9 @error('phone_number_3') is-invalid @enderror" placeholder="" aria-label="" name="phone_number_3" aria-describedby="addon-wrapping">');
        }
    })
</script>
@stop
